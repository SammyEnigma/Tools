...
namespace RedisTool.Base
{
    public class RedisBase : IRedisBase
    {
        ...
        public RedisBase(string host, int port, string pwd, int db)
        {
            // todo: 从配置文件中读取
            _db = db;
            _host = host;
            ...
            _multiplexer = ConnectionMultiplexer.Connect(_host + ":" + _port + ",password=" + _pwd + ",abortConnect=false,connectTimeout=10000");
